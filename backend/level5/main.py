...
def get_rental_price(rental, car):
    ...
    # Additional fees for the options
    options_fees = {
        'gps': 500,
        'baby_seat': 200,
    ...
    # Add the additional fees for the options
    for option in rental['options']:
        price += options_fees[option['type']]*nb_days
        if option['type']=='gps' or option['type']=='baby_seat':
            owner_fee+=options_fees[option['type']]*nb_days
        elif option['type']=='additional_insurance':
            drivy_fee+=options_fees[option['type']]*nb_days

    return {
        "price": round(price),
        "commission": {
            "owner_fee": round(owner_fee),
...
def get_rental_actions(rental, price):
    ...
    actors = [
        ...
        {"who": "owner", "type": "credit", "amount": round(price['commission']['owner_fee'])},
...
for rental in data['rentals']:
    ...
    # Add the selected options to the rental
    rental['options'] = [option for option in data['options'] if option['rental_id'] == rental['id']]
    ...
    output_data['rentals'].append({
        "id": rental['id'],
        "options": [option['type'] for option in rental['options']],
        "actions": actions
    })
